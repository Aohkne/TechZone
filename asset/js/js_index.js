...
setInterval(() => {
  ...
  if (second.textContent > 0) {
    ...
    //min
    min.textContent =
      min.textContent < 10 ? "0" + (min.textContent - 1) : min.textContent - 1;
  } else if (second.textContent <= 0 && min.textContent <= 0) {
    // hour
    second.textContent = 60;
    min.textContent = 60;
...
function show(Click, Effect) {
  ...
  document.onclick = (event) => {
    if (
      event.target != document.querySelector("#button_next") &&
      !userItem[0].contains(event.target)
    ) {
      Effect.style.display = "none";
    }
  };
